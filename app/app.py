...
def create_certificate():
    try:
        ...
        cert = ParamsSelfSignedCert(surname, name, department, position, common_name, org_name, org_country, org_region, org_locality, org_address)
...
def revoke_certificate_page():
    try:
        ...
        cursor.execute("SELECT serial_number, is_revoked, reason FROM certificates")
        ...
        for cert in certificates:
            certs_data.append({
                'serial_number': str(cert['serial_number']),  # преобр.UUID в строку
    ...
    try:
        ...
        try:
            ...
            for cert_serial in certs_to_revoke:
                cursor.execute(
                    "UPDATE certificates SET is_revoked = TRUE WHERE serial_number = %s",
                    (cert_serial,)
                )
